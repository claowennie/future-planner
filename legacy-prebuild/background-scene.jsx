...
function bgMix(a,b,t){ const A=bgHexArr(a),B=bgHexArr(b); return bgToHex([0,1,2].map(i=>A[i]+(B[i]-A[i])*t)); }
...
/* ---- tiny seeded RNG --------------------------------------------------- */
function bgRng(seed){ let s=seed>>>0; return ()=>{ s=(s+0x6D2B79F5)>>>0; let t=s; t=Math.imul(t^(t>>>15),t|1); t^=t+Math.imul(t^(t>>>7),t|61); return ((t^(t>>>14))>>>0)/4294967296; }; }
