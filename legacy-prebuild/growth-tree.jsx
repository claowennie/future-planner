...
const STAGE_NAMES = ['种子', '嫩芽', '幼苗', '小树', '成长之树', '茂盛之树', '参天古树'];
...
const STAGE_THRESHOLD_LABELS = ['第 0 天', '1–3 天', '4–7 天', '8–14 天', '15–30 天', '31–60 天', '61 天以上'];
    ...
    <div className="auth-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="auth-modal growth-stages-modal">
        ...
        <div className="auth-head">
          <div className="auth-title serif">成长树 · 七个阶段</div>
          <div className="auth-sub">连续打卡越久，树长得越茂盛。你现在在「{STAGE_NAMES[currentStage]}」。</div>
        </div>
          ...
          {[0,1,2,3,4,5,6].map(s => (
            <div className={`growth-stage-cell ${s === currentStage ? 'current' : ''}`} key={s}>
              ...
              <div className="growth-stage-meta">
                <div className="growth-stage-name">{STAGE_NAMES[s]}{s === currentStage && <span className="growth-stage-here"> · 你在这</span>}</div>
...
function GrowthStatusPill({ streak }) {
  ...
  return (
    <>
    <div className="growth-pill growth-pill-clickable" onClick={() => setGalleryOpen(true)} title="点击查看成长树的全部阶段">
      ...
      <div className="growth-pill-text">
        ...
        <div className="growth-pill-streak">
          ...
          <span className="growth-pill-unit">天连续</span>
          {next && <span className="growth-pill-next">· 还有 {next - (streak || 0)} 天到 {STAGE_NAMES[stage + 1]}</span>}
          {!next && <span className="growth-pill-next">· 你已抵达最高阶段</span>}
        </div>
      </div>
    </div>
