...
async function deleteAccount() {
  ...
  try {
    ...
    try {
      const { data: files } = await sbClient.storage.from('note-images').list(user.id, { limit: 1000 });
      if (files && files.length) {
        await sbClient.storage.from('note-images').remove(files.map((f) => `${user.id}/${f.name}`));
      }
    } catch { /* 没有图片或列举失败不挡注销 */ }
    // 2) 云端数据行
    ...
    if (error) throw error;
    // 4) 本机清理（与登出同款；planner_backups_v1 留着当本机最后的后悔药）
