...
function hueFor(tr) {
  if (tr && Number.isFinite(Number(tr.hue))) return ((Math.round(Number(tr.hue)) % 360) + 360) % 360;
  const s = `${(tr && tr.artist) || ''}${(tr && tr.title) || ''}`;
  let h = 0;
...
function RadioView() {
  ...
  const playAt = async (i, tracks = queue) => {
    ...
    if (tr.intro) setLog((l) => [...l, { role: 'claudio', text: tr.intro }]);
    await speak(tr.intro, tr.introAudio);
    if (token !== playTokenRef.current) return; // 期间用户切了别的，放弃

    const a = musicRef.current;
    if (tr.url) {
      // 本地曲库给的是 /media/... 相对路径，补上中枢地址；网易/QQ 给的是完整 http 链接
      a.src = tr.url.startsWith('http') ? tr.url : `${CLAUDIO_BASE}${tr.url}`;
      a.play().catch(() => {});
  ...
  const send = async (text) => {
    ...
    try {
      ...
      tracks.forEach((tr) => prefetchTts(tr.introAudio));
      setThinking(false);
    ...
    <div className="main-inner radio">
      ...
      {queue.length > 1 && (
        <div className="radio-queue">
          ...
          {queue.map((tr, i) => (
            <button key={i} className={`radio-queue-item ${i === idx ? 'active' : ''}`} onClick={() => playAt(i)} style={{ '--rad-h': hueFor(tr) }}>
              <span className="radio-queue-dot" />
              <span className="radio-queue-t">{tr.title}</span>
              <span className="radio-queue-a">{tr.artist}</span>
            </button>
          ))}
        </div>
      )}
